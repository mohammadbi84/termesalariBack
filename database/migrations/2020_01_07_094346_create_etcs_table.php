...
class CreateEtcsTable extends Migration
{
    ...
    public function up()
    {
        Schema::create('etcs', function (Blueprint $table) {
            ...
            $table->unsignedBigInteger('design_color_id')->index('design_color_id')->comment('رنگ محصول');
            $table->string('dimensions','400')->comment('ابعاد');
            $table->string('size','50')->comment('ابعاد');
            $table->string('weight','50')->comment('وزن تقریبی');
            $table->string('kind','50')->comment('جنس');
            $table->string('contains','400')->comment('مشتمل بر');
            ...
            // $table->string('offType','20')->comment('نوع تخفیف')->default('percent');
