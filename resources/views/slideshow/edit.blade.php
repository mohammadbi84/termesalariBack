	...
	<section class="content">
		<div class="row">
			...
			<div class="card col-md-10 col-sm-12">
				...
				<form class="form" role="form" action="{{ route('slideshow.update',[$slideshow]) }}" method="POST" enctype="multipart/form-data">
					<div class="card-body">
						@csrf
						@method('put')
						<div class="form-group @error('position') is-invalid @enderror">
							<label for="position">موقعیت اسلایدشو</label>
		                  	<select name="position" id="position" class="form-control select2 select2-hidden-accessible " style="width: 100%;">
			                    ...
			                    <option value="homeStore-A" @if (old('position',$slideshow->position) == 'homeStore-A')) selected @endif >صفحه اصلی - موقعیت اول</option>
			                  	<option value="homeStore-B" @if (old('position',$slideshow->position) == 'homeStore-B')) selected @endif >صفحه اصلی - موقعیت دوم</option>
		                  	</select>
		                </div>

		                <div class="form-group">
							<label for="title">عنوان</label>
							<input type="text" name="title" id="title" class="form-control @error('title') is-invalid @enderror" placeholder="لطفا عنوان  تصویر را وارد کنید." value="{{old('title',$slideshow->title)}}" maxlength="100">
							@error('title')
						...
						<div class="form-group">
							...
							<textarea name="description" id="description" class="form-control @error('description') is-invalid @enderror" placeholder="لطفا توضیحات  تصویر را وارد کنید." maxlength="300">{{old('description',$slideshow->description)}}</textarea>
							@error('description')
							    <div class="invalid-feedback">{{$message}}</div>
							@enderror
						</div>
				        ...
				        <div class="form-group">
							...
							<input type="text" name="link" id="link" class="form-control @error('link') is-invalid @enderror" placeholder="لطفا آدرس لینک تصویر را وارد کنید." value="{{old('link',$slideshow->link)}}">
							@error('link')
							    <div class="invalid-feedback">{{$message}}</div>
							@enderror
						</div>
				        ...
				        <div class="form-group">
							...
							<input type="text" name="order" id="order" class="form-control @error('order') is-invalid @enderror" placeholder="لطفا عدد مربوط به ترتیب تصویر را وارد کنید." value="{{old('order',$slideshow->order)}}">
							@error('order')
							    <div class="invalid-feedback">{{$message}}</div>
							@enderror
						</div>
