        ...
        <div class="col">
            ...
            <div class="card">
                <div class="card-header">
                    <h3 class="card-title"><span>ویرایش محصول {{ $bedcover->category->title }} طرح
                            {{ $bedcover->color_design->design->title }} رنگ
                            {{ $bedcover->color_design->color->color }}</span></h3>
                </div>
                <!-- /.card-header -->
                <!-- form start -->
                {{-- @dump($errors) --}}
                <form role="form" id="editForm" action="{{ route('bedcover.update', [$bedcover]) }}" method="POST"
...
@push('js')
    ...
    <script>
        ...
        let tags = @json($bedcover->tags->pluck('name')->values());
                    ...
                    success: function(data) {
                        if (data.length > 0) {
                            ...
                            data.forEach((element) => {
                                if ("{{ old('color_id', $bedcover->color_design->color->id) }}" ==
                                    element.id)
