...
@section('title','پنل مدیریت | محصولات روتختی')
      ...
      <div class="row">
        <div class="col-12">
            ...
            <div class="card-header">
              	<a href="{{ route('bedcover.create') }}" class="pr-3 pl-3 pt-2 pb-2 btn btn-flat btn-danger float-left " title="محصول جدید">+</a>
              ...
              <table id="dataTable-table" class="table table-striped display nowrap dataTable" style="width:100%;text-align: center;" cellspacing="0">
                ...
                {{-- {{ dd($bedcover) }} --}}
	                <tr>
	                	<td>
	                		<label>
		                    	<input type="checkbox" data-value = "{{$bedcover->id}}" class="flat-red checkbox">
		                  	</label>
	                	</td>
						<td>{{$loop->iteration}}</td>
						<td>{{$bedcover->code}}</td>
						<td>
							@if($bedcover->images->first())
							@php
								$image = $bedcover->images->first();
							@endphp
							
							<img src="{{ asset('storage/images/thumbnails/'. $bedcover->images->first()->name)}}" alt="" class="img-circle img-size-50 mr-2">
						...
						<td>
							@php
								...
								$designColor = $bedcover->color_design->color->color; 
								print $designColor; 
							 @endphp
						</td>
						...
						<td>
							<div class="br-wrapper br-theme-fontawesome-stars float-left">
								{{-- {{$bedcover->grades->avg('grade') ?? '0'}} --}}
								<select class="showGrade{{$key}}" data-value="{{ $bedcover->grades->avg('grade') ?? '0' }}"> 
						...
						<td>
							@if($bedcover->visibility == 0)
								<a class="changeVisibility" href="#" data-id="{{$bedcover->id}}"><i class="fas fa-close danger-color"></i></a> 
							@else
								<a class="changeVisibility" href="#" data-id="{{$bedcover->id}}"><i class="fas fa-check success-color"></i> </a>
							@endif
						</td>
						<td>
							{{-- <a href="{{ route('bedcover.duplicate',[$bedcover]) }}"><i class="fas fa-copy text-primary"></i></a> --}}
							<a href="{{ route('bedcover.duplicate',[$bedcover]) }}" class="btn btn-outline-secondary btn-flat btn-sm"><i class="fas fa-copy"></i> کپی  </a>
						</td>
						<td><a href="{{route('bedcover.show',[$bedcover])}}" target="blank" class="btn btn-outline-info btn-flat btn-sm"><i class="fas fa-info-circle"></i> جزئیات </a></td>
						<td><a href="{{route('bedcover.edit',[$bedcover])}}" class="btn btn-outline-primary btn-flat btn-sm"><i class="fas fa-edit"></i> ویرایش </a></td>
						<td><a href="{{route('comment.product',['Bedcover',$bedcover->id])}}" class="btn btn-outline-primary btn-flat btn-sm"><i class="fas fa-comment"></i> نظرات </a></td>

						<td>
							<form class="del-form" action="{{route('bedcover.destroy', [$bedcover])}}" method="post">
								@csrf
								@method('delete')
								<a href="#" data-id="{{$bedcover->id}}" data-model="Bedcover" class="btn btn-outline-danger btn-flat btn-sm delete"><i class="fas fa-trash-alt"></i> حذف </a>
							</form>
						</td>
	                </tr>
	            @endforeach
...
//-------------------Show Grade-----------------------------
	{{-- @foreach($bedcovers as $key=>$bedcover)
		$('.showGrade{{$key}}').barrating({
		 	theme:'fontawesome-stars-o',
			initialRating: "{{$bedcover->grades->avg('grade') ?? '0'}}",
		 	readonly:true,
		});
	@endforeach --}}
//-------------------Delete Product------------------
	...
	$(document).on('click', '.changeVisibility', function(event){
		event.preventDefault();
		var url = "{{ route('bedcover.changeVisibility') }}";
		var id = $(this).data("id");
		var $thiz = $(this);
		$.ajax({
		    type: 'POST',
...
//----------change Statuse group--------------------------------
	$('#changeVisibilityGroup').click(function(event){
		event.preventDefault();
		var items = [];
		var url = "{{ route('bedcover.changeVisibilityGroup') }}"
