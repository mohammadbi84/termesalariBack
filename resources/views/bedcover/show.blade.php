...
@section('title', $title ." طرح  ". $bedcover->color_design->design->title ." رنگ  ". $bedcover->color_design->color->color)
      ...
      <div class="col-md-12" style="margin: 0 auto;">
        <div class="card">
          ...
          <div class="card-header">
            <h3 class="card-title">
            	<span id="title"><a href="{{ route('bedcover.storeIndex') }}" style="color: #18d26e;">محصولات روتختی</a> / {{ $bedcover->category->title }} طرح {{ $bedcover->color_design->design->title }} رنگ {{ $bedcover->color_design->color->color }}</span>
          		...
          		<div class="mt-3" style="width: 320px; margin: auto auto;">
          			<div class="card card-sale card-danger card-outline ">
		              	...
		              	<div class="card-body">
		                	...
		                	<p>	
		                	<div class="row">
		                		<input @if($bedcover->quantity > 0) id="addToCart" @endif type="submit" value="افزودن به سبد خرید" class="btn btn-block btn-danger btn-lg @if($bedcover->quantity==0) disabled @endif" data-id = "{{$bedcover->id}}" data-moddel = "Bedcover" data-design="{{ $bedcover->color_design->design->title }}" data-color="{{ $bedcover->color_design->color->color }}" data-title="{{ $bedcover->title }}" data-price="{{ $prices->price }}" data-pay="{{ $price }}" data-off="{{ $off }}" data-local="{{ $prices->local }}" >
		                	</div>
		                	</p>

							<div class="br-wrapper br-theme-fontawesome-stars">
	                				امتیاز : 
							  <select id="showGrade" data-model="Bedcover" data-current-rating="4"> <!-- now hidden -->
...
{{-- {{ dd($bedcover->favorites->where('user_id',Auth::id())) }} --}}
							...
							<a href="#" id="addToFavorite" data-id="{{ $bedcover->id }}" data-model="Bedcover" class="float-left mt-1" title="افزودن به لیست علاقه مندی ها" style="font-size: 1.5rem"><i class="@if($bedcover->favorites->where('user_id',Auth::id())->count() > 0 ) fa fa-heart @else far fa-heart @endif"></i></a>

							<a href="" id="compare" title="افزودن به لیست مقایسه" class="float-left ml-2 mt-2" data-id="{{ $bedcover->id }}" data-model="Bedcover" style="vertical-align: middle;"><img src="{{ asset('/storetemplate/dist/img/compare.png') }}"></a>
                ...
                <div class="tab-content">
                  	<div class="tab-pane active" id="tab_1">
                  		<table class="table table-striped table-data">
	                  		<tr>
	                    		<th colspan="2" style="color: #ef3a4e;">{{ $bedcover->category->title }} طرح {{ $bedcover->color_design->design->title }} رنگ {{ $bedcover->color_design->color->color }}</th>
	                  		</tr>
	                  		<tr>
	                  			<td>کد محصول</td>
	                  			<td>{{$bedcover->code}}</td>
			            ...
			            {{-- <div class="card card-widget"> --}}
			              ...
			              {{-- <div class="card-body"> --}}
			                ...
			                <div class="br-wrapper br-theme-fontawesome-stars" style="margin-bottom: 20px;">
							  ...
							  <select id="setGrade" data-model="Bedcover" data-id="{{ $bedcover->id }}"> <!-- now hidden -->
							    <option value="1">1</option>
							    <option value="2">2</option>
							    <option value="3">3</option>
							    <option value="4">4</option>
								...
								@if(Auth::check())
									<form action="" method="">
									    ...
									    <div class="img-push">
									    	...
									    	<br>
									  		<input type="submit" id="saveComment" data-model="Bedcover" data-id="{{$bedcover->id}}" value="ثبت" class="btn btn-danger">
									  	</div>
									</form>
								@else
						            ...
						            <div class="box-product">
						                <a href="{{ route('bedcover.show',[$likeBedcover->id]) }}">
						                	<div class="img-wrapper mb-2">
						                      	<img alt="Product" src="{{asset('/storage/images/thumbnails/'.$likeBedcover->images->sortby('ordering')->first()->name)}}" class="w-100">
						                	</div>
						                </a>

						                <h6>
						                  <a href="{{ route('bedcover.show',[$likeBedcover->id]) }}" class="title">{{ $likeBedcover->category->title }} طرح {{ $likeBedcover->color_design->design->title}} رنگ {{ $likeBedcover->color_design->color->color }}</a>
						                </h6>
						                <div class="price">
						                    @php
						                ...
						                <div style="clear: both;">
						                  ...
						                  @endif
						                  <small @if($likeBedcover->quantity > 5) style="color: #000" @else style="color: #ef3a4e" @endif>
						                  @if($likeBedcover->quantity == 0) اتمام موجودی در انبار 
						                    @elseif($likeBedcover->quantity <= 5)کمتر از 5 عدد موجود می باشد  .
						                  @endif
						                  </small>
						                </div>
