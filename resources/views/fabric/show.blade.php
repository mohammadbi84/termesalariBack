...
@section('title', $title ." طرح  ". $fabric->color_design->design->title ." رنگ  ". $fabric->color_design->color->color)
      ...
      <div class="col-md-12" style="margin: 0 auto;">
        <div class="card">
          ...
          <div class="card-header">
            <h3 class="card-title">
            	<span id="title"><a href="{{ route('fabric.storeIndex') }}" style="color: #18d26e;">محصولات پارچه ترمه</a> / {{ $fabric->category->title }} طرح {{ $fabric->color_design->design->title }} رنگ {{ $fabric->color_design->color->color }}</span>
				...
				@php
          		...
          		<div class="mt-3" style="width: 320px; margin: auto auto;">
          			<div class="card card-sale card-danger card-outline ">
		              	...
		              	<div class="card-body">
		              		<p class="quantity" data-id="{{$fabric->quantity}}" style="color: #ef3a4e;">
...
{{-- {{ dd($fabric->favorites->where('user_id',Auth::id())) }} --}}
                ...
                <div class="tab-content">
                  	<div class="tab-pane active" id="tab_1">
                  		<table class="table table-striped table-data">
	                  		<tr>
	                    		<th colspan="2" style="color: #ef3a4e;">{{ $fabric->category->title }} طرح {{ $fabric->color_design->design->title }} رنگ {{ $fabric->color_design->color->color }}</th>
	                  		</tr>
	                  		<tr>
	                  			<td>کد محصول</td>
	                  			<td>{{$fabric->code}}</td>
	                  		...
	                  		<tr>
	                    		...
	                    		<td>{{ $fabric->color_design->design->countOfColor }} رنگ</td>
	                  		</tr>
	                  		...
	                  		<tr>
	                    		<td>وزن تقریبی</td>
	                    		<td class="multi-line">{{$fabric->weight}}</td>
	                  		</tr>
	                  		<tr>
	                    		<td>جنس محصول</td>
	                    		<td>{{$fabric->kind}}</td>
								...
								@if(Auth::check())
									<form action="" method="">
									    ...
									    <div class="img-push">
									    	...
									    	<br>
									  		<input type="submit" id="saveComment" data-model="Fabric" data-id="{{$fabric->id}}" value="ثبت" class="btn btn-danger">
									  	</div>
									</form>
								@else
				          	...
				          	@foreach($likeFabrics as $key=>$likeFabric)
				          		...
				          		<div class="box-product-outer">
						            <div class="box-product">
						                ...
						                <h6>
						                  <a href="{{ route('fabric.show',[$likeFabric->id]) }}" class="title">{{ $likeFabric->category->title }} طرح {{ $likeFabric->color_design->design->title}} رنگ {{ $likeFabric->color_design->color->color }}</a>
						                </h6>
						                <div class="price">
						                    @php
						                    	$prices = $likeFabric->prices->where("local","تومان")->first();
						                    @endphp
						                    <div>
						                      @if($prices->offPrice > 0)
						                ...
						                <div style="clear: both;">
						                  @if($likeFabric->quantity > 0 and $likeFabric->quantity <= 5)
	...
	$(function(){
		...
		@foreach($likeFabrics as $key=>$likeFabric)
	        $('.showGradeLikeFabric{{$key}}').barrating({
	          ...
	          initialRating: "{{$likeFabric->grades->avg('grade') ?? '0'}}",
	          readonly:true,
	        });
      	@endforeach
