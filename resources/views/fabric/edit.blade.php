	...
	<div class="col">
	    ...
	    <div class="card">
	      <div class="card-header">
	        <h3 class="card-title"><span>ویرایش  محصول  {{ $fabric->category->title}} طرح {{$fabric->color_design->design->title}} رنگ {{$fabric->color_design->color->color}}</span></h3>
	      </div>
	      <!-- /.card-header -->
	      <!-- form start -->
	      {{-- @dump($errors) --}}
	      <form role="form" id="editForm" action="{{route('fabric.update',[$fabric])}}" method="POST" enctype="multipart/form-data">
	      	@csrf
	      	@method('put')
	        <div class="card-body">
	        	...
	        	<div class="form-group">
					...
					<input type="text" name="code" class="form-control @error('code') is-invalid @enderror" placeholder="کد محصول را وارد کنید ." value="{{old('code',$fabric->code)}}" autofocus="autofocus">
					@error('code')
					...
					<label for="category_id">دسته بندی محصول</label>
                  	<select name="category_id" id="category_id" class="form-control select2 select2-hidden-accessible " style="width: 100%;">
                  		...
                  		@foreach($categories as $category)
                  			<option value="{{ $category->id }}" @if($category->id == old('category_id',$fabric->category->id)) selected @endif >{{ $category->title }}</option>
                  		@endforeach
                  	</select>
				...
				<div class="form-group">
                    ...
                    <select name="color_id" id="color_id" class="form-control @error('color_id') is-invalid @enderror">
                    	@foreach($colors as $color)
                    		<option @if (old('color_id', $fabric->color_design->color->id) == $color->id ) selected @endif value="{{ $color->id }}">{{ $color->color }}</option>
                    	@endforeach
                    </select>
                    @error('color_id')
					    <div class="invalid-feedback">{{$message}}</div>
				...
				<div class="form-group">
                    ...
                    <select name="kind" class="form-control @error('kind') is-invalid @enderror">
                    	{{-- <option value="">جنس محصول را انتخاب کنید .</option> --}}
                      	<option  @if (old('kind',$fabric->kind) == 'ابریشم مصنوعی (ویسکوز ریون)') selected @endif value="ابریشم مصنوعی (ویسکوز ریون)">ابریشم مصنوعی (ویسکوز ریون)</option>
				...
				<div class="form-group">
                    ...
                    <select name="washable" class="form-control @error('washable') is-invalid @enderror">
                    	<option value="">قابلیت شستشو</option>
                      	<option @if (old('washable',$fabric->washable) == 'توسط دست') selected @endif value="توسط دست">توسط دست</option>
						<option @if (old('washable',$fabric->washable) == 'دارد ( ترجیحا خشکشویی)') selected @endif value="دارد ( ترجیحا خشکشویی)">دارد ( ترجیحا خشکشویی)</option>
						<option @if (old('washable',$fabric->washable) == 'ندارد') selected @endif value="ندارد">ندارد</option>
                    </select>
                    @error('washable')
					    <div class="invalid-feedback">{{$message}}</div>
					@enderror
                ...
                <div class="form-group">
					...
					<input type="text" name="quantity" class="form-control @error('quantity') is-invalid @enderror" placeholder="مثلاً 200" value="{{old('quantity',$fabric->quantity)}}">
					@error('quantity')
					    <div class="invalid-feedback">{{$message}}</div>
					@enderror
				</div>
...
<script>
        ...
        let tags = @json($fabric->tags->pluck('name')->values());
	        	...
	        	@foreach($images as $image)
		        	{
		        		...
		        		extra: {
		        		 ...
		        		 {{--size: '{{ \File::size(public_path().'/storage/fabrics/'.$image['name']) }}',--}}
				...
				success: function(data)
				{
					if(data.length > 0){
						...
						data.forEach((element) => {
							if("{{ old('color_id', $fabric->color_design->color->id) }}" == element.id)
