      ...
      <div class="row">
        <div class="col-12">
            ...
            <div class="card-header">
              	<a href="{{ route('fabric.create') }}" class="pr-3 pl-3 pt-2 pb-2 btn btn-flat btn-danger float-left " title="محصول جدید">+</a>
              ...
              <table id="dataTable-table" class="table table-striped display nowrap dataTable" style="width:100%;text-align: center;" cellspacing="0">
                ...
                {{-- {{ dd($fabric) }} --}}
	                <tr>
	                	<td>
	                		<label>
		                    	<input type="checkbox" data-value = "{{$fabric->id}}" class="flat-red checkbox">
		                  	</label>
	                	</td>
						<td>{{$loop->iteration}}</td>
						<td>{{$fabric->code}}</td>
						<td>
							@if($fabric->images->first())
							@php
								$image = $fabric->images->first();
							@endphp
							
							<img src="{{ asset('storage/images/thumbnails/'. $fabric->images->first()->name)}}" alt="" class="img-circle img-size-50 mr-2">

							@endif
							{{ $fabric->title }}
						</td>
						{{-- <td>{{$fabric->category->title}}</td> --}}
						<td>{{ $fabric->color_design->design->title }}</td>
						<td>
							@php
								$prices = $fabric->prices->where('local','تومان')->first();
								$designColor = $fabric->color_design->color->color; 
								print $designColor; 
							 @endphp
						</td>
						...
						<td>
							<div class="br-wrapper br-theme-fontawesome-stars float-left">
								{{-- {{$fabric->grades->avg('grade') ?? '0'}} --}}
								<select class="showGrade{{$key}}" data-value="{{ $fabric->grades->avg('grade') ?? '0' }}"> 
						...
						<td>
							@if($fabric->visibility == 0)
								<a class="changeVisibility" href="#" data-id="{{$fabric->id}}"><i class="fas fa-close danger-color"></i></a> 
							@else
								<a class="changeVisibility" href="#" data-id="{{$fabric->id}}"><i class="fas fa-check success-color"></i> </a>
							@endif
						</td>
						<td>

							<a href="{{ route('fabric.duplicate',[$fabric]) }}" class="btn btn-outline-secondary btn-flat btn-sm"><i class="fas fa-copy"></i> کپی  </a>
						</td>
						<td><a href="{{route('fabric.show',[$fabric])}}" target="blank" class="btn btn-outline-info btn-flat btn-sm"><i class="fas fa-info-circle"></i> جزئیات </a></td>
						<td><a href="{{route('fabric.edit',[$fabric])}}" class="btn btn-outline-primary btn-flat btn-sm"><i class="fas fa-edit"></i> ویرایش </a></td>
						<td><a href="{{route('comment.product',['Fabric',$fabric->id])}}" class="btn btn-outline-primary btn-flat btn-sm"><i class="fas fa-comment"></i> نظرات </a></td>

						<td>
							<form class="del-form" action="{{route('fabric.destroy', [$fabric])}}" method="post">
								@csrf
								@method('delete')
								<a href="#" data-id="{{$fabric->id}}" data-model="Fabric" class="btn btn-outline-danger btn-flat btn-sm delete"><i class="fas fa-trash-alt"></i> حذف </a>
							</form>
						</td>
	                </tr>
	            @endforeach
...
//-------------------Delete Product------------------
	...
	$(document).on('click', '.changeVisibility', function(event){
		event.preventDefault();
		var url = "{{ route('fabric.changeVisibility') }}";
		var id = $(this).data("id");
		var $thiz = $(this);
		$.ajax({
		    type: 'POST',
...
//----------change Statuse group--------------------------------
	$('#changeVisibilityGroup').click(function(event){
		event.preventDefault();
		var items = [];
		var url = "{{ route('fabric.changeVisibilityGroup') }}"
