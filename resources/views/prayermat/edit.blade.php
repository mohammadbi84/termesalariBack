	...
	<div class="col-12">
	    ...
	    <div class="card">
	      <div class="card-header">
	        <h3 class="card-title"><span>ویرایش  محصول  {{ $prayermat->category->title}} طرح {{$prayermat->color_design->design->title}} رنگ {{$prayermat->color_design->color->color}}</span></h3>
	      </div>
	      <!-- /.card-header -->
	      <!-- form start -->
	      {{-- @dump($errors) --}}
	      <form role="form" id="editForm" action="{{route('prayermat.update',[$prayermat])}}" method="POST" enctype="multipart/form-data">
	      	@csrf
	      	@method('put')
	        <div class="card-body">
	        	...
	        	<div class="form-group">
					...
					<input type="text" name="code" class="form-control @error('code') is-invalid @enderror" placeholder="کد محصول را وارد کنید ." value="{{old('code',$prayermat->code)}}" autofocus="autofocus">
					@error('code')
					...
					<label for="category_id">دسته بندی محصول</label>
                  	<select name="category_id" id="category_id" class="form-control select2 select2-hidden-accessible " style="width: 100%;">
                  		...
                  		@foreach($categories as $category)
                  			<option value="{{ $category->id }}" @if($category->id == old('category_id',$prayermat->category->id)) selected @endif >{{ $category->title }}</option>
                  		@endforeach
                  	</select>
				...
				<div class="form-group">
                    ...
                    <select name="color_id" id="color_id" class="form-control @error('color_id') is-invalid @enderror">
                    	@foreach($colors as $color)
                    		<option @if (old('color_id', $prayermat->color_design->color->id) == $color->id ) selected @endif value="{{ $color->id }}">{{ $color->color }}</option>
                    	@endforeach
                    </select>
                    @error('color_id')
					    <div class="invalid-feedback">{{$message}}</div>
				...
				<div class="form-group">
					...
					<textarea name="contains" class="form-control @error('contains') is-invalid @enderror" rows="3" placeholder="مثلا شامل یک رومیزی  مربع و دو رومیزی عسلی">{{old('contains',$prayermat->contains)}}</textarea>
				...
				<div class="form-group">
                    ...
                    <select name="kind" class="form-control @error('kind') is-invalid @enderror">
                    	{{-- <option value="">جنس محصول را انتخاب کنید .</option> --}}
                      	<option  @if (old('kind',$prayermat->kind) == 'ابریشم مصنوعی (ویسکوز ریون)') selected @endif value="ابریشم مصنوعی (ویسکوز ریون)">ابریشم مصنوعی (ویسکوز ریون)</option>
						<option  @if (old('kind',$prayermat->kind) == 'ابریشم مصنوعی') selected @endif value="ابریشم مصنوعی">ابریشم مصنوعی</option>
                    </select>
                    @error('kind')
					    <div class="invalid-feedback">{{$message}}</div>
					@enderror
                ...
                <div class="form-group">
                    ...
                    <select name="sewingType" class="form-control @error('sewingType') is-invalid @enderror">
                    	{{-- <option value="">نوع دوخت را انتخاب کنید .</option> --}}
                      	<option @if (old('sewingType',$prayermat->sewingType) == 'مغزی دوزی') selected @endif value="مغزی دوزی">مغزی دوزی</option>
                ...
                <div class="form-group">
                    ...
                    <select name="haveEster" id="haveEster" class="form-control @error('haveEster') is-invalid @enderror">
                    	{{-- <option value="">آنتخاب ویژگی آستر</option> --}}
                      	<option @if (old('haveEster',$prayermat->haveEster) == 'دارد') selected @endif value="دارد">دارد</option>
						<option @if (old('haveEster',$prayermat->haveEster) == 'ندارد') selected @endif value="ندارد">ندارد</option>
                    </select>
                    @error('haveEster')
					    <div class="invalid-feedback">{{$message}}</div>
					@enderror
				...
				<div class="form-group">
					...
					<input disabled="disabled" type="text" name="kindOfEster" id="kindOfEster" class="form-control @error('kindOfEster') is-invalid @enderror" placeholder="مثلاً ساتن مرغوب" value="{{old('kindOfEster',$prayermat->kindOfEster)}}"  >
					@error('kindOfEster')
					    <div class="invalid-feedback">{{$message}}</div>
					@enderror
				</div>
				...
				<div class="form-group">
                    ...
                    <select name="washable" class="form-control @error('washable') is-invalid @enderror">
                    	<option value="">قابلیت شستشو</option>
                      	<option @if (old('washable',$prayermat->washable) == 'توسط دست') selected @endif value="توسط دست">توسط دست</option>
						<option @if (old('washable',$prayermat->washable) == 'دارد ( ترجیحا خشکشویی)') selected @endif value="دارد ( ترجیحا خشکشویی)">دارد ( ترجیحا خشکشویی)</option>
                ...
                <div class="form-group">
					...
					<input type="text" name="quantity" class="form-control @error('quantity') is-invalid @enderror" placeholder="مثلاً 200" value="{{old('quantity',$prayermat->quantity)}}">
					@error('quantity')
					    <div class="invalid-feedback">{{$message}}</div>
					@enderror
				</div>
	        ...
	        <div class="card-footer">
	          ...
	          <a href="{{ route('prayermat.index') }}" class="btn btn-flat btn-secondary">بازگشت</a>
	        </div>
	      </form>
	    </div>
	    <!-- /.card -->
...
<script>
        ...
        let tags = @json($prayermat->tags->pluck('name')->values());
				...
				success: function(data)
				{
					if(data.length > 0){
						...
						data.forEach((element) => {
							if("{{ old('color_id', $prayermat->color_design->color->id) }}" == element.id)
