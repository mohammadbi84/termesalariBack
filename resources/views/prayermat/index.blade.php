...
@section('title', 'پنل مدیریت | محصولات سجاده و جانماز')
        ...
        <div class="row">
            ...
            <div class="card">
                ...
                <div class="card-body">
                        ...
                        <tbody>
                            ...
                            @foreach ($prayermats as $key => $prayermat)
                                ...
                                <tr>
                                    <td>
                                        <label>
                                            <input type="checkbox" data-value = "{{ $prayermat->id }}"
                                    ...
                                    <td>
                                        @php
                                            ...
                                            $designColor = $prayermat->color_design->color->color;
                                            print $designColor;
                                        @endphp
                                    </td>
                                    ...
                                    <td>
                                        <div class="br-wrapper br-theme-fontawesome-stars float-left">
                                            {{-- {{$prayermat->grades->avg('grade') ?? '0'}} --}}
                                            <select class="showGrade{{ $key }}"
                                                data-value="{{ $prayermat->grades->avg('grade') ?? '0' }}">
                                    ...
                                    <td>
                                        ...
                                        <a href="{{ route('prayermat.duplicate', [$prayermat]) }}"
                                    ...
                                    <td><a href="{{ route('prayermat.show', [$prayermat]) }}" target="blank"
                                            class="btn btn-outline-info btn-flat btn-sm"><i
                                                class="fas fa-info-circle"></i> جزئیات </a></td>
                                    <td><a href="{{ route('prayermat.edit', [$prayermat]) }}"
                                            class="btn btn-outline-primary btn-flat btn-sm"><i class="fas fa-edit"></i>
                                            ویرایش </a></td>
                                    <td><a href="{{ route('comment.product', ['Prayermat', $prayermat->id]) }}"
                                            class="btn btn-outline-primary btn-flat btn-sm"><i
                                                class="fas fa-comment"></i> نظرات </a></td>

                                    <td>
                                        <form class="del-form"
                                            action="{{ route('prayermat.destroy', [$prayermat]) }}" method="post">
                                            @csrf
                                            @method('delete')
                                            <a href="#" data-id="{{ $prayermat->id }}" data-model="Prayermat"
                    ...
                    <div class="buttons">
                        <a href="" id="changeVisibilityGroup" class="btn btn-primary btn-flat">نمایش / عدم
                            نمایش</a>
                    </div>
                </div>
                <!-- /.card-body -->
...
<script>
	...
	@endforeach --}}
        ...
        $(document).on('click', '.changeVisibility', function(event) {
            event.preventDefault();
            var url = "{{ route('prayermat.changeVisibility') }}";
            var id = $(this).data("id");
            var $thiz = $(this);
            $.ajax({
                type: 'POST',
        ...
        $('#changeVisibilityGroup').click(function(event) {
            event.preventDefault();
            var items = [];
            var url = "{{ route('prayermat.changeVisibilityGroup') }}"
            $("#dataTable-table").find("div[aria-checked='true']").children("input").each(function(
                index) {
            ...
            $.ajax({
                ...
                success: function(data) {
                    if (data.res == "error") {
                        ...
                        title = "عملیات با موفقیت انجام شد.";
                        $("#dataTable-table").find("div[aria-checked='true']").each(
                            function(index) {
                                $(this).find('.icheckbox_flat-blue').iCheck('uncheck');
                                $(this).removeClass("bg-skyblue");
                                var $thiz = $(this).parents("tr").find(
                                    "a[class='changeVisibility']").children("i");
