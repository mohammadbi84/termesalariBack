      ...
      <div class="row">
        <div class="col-12">
            ...
            <div class="card-header">
              	<a href="{{ route('prayermat.create') }}" class="pr-3 pl-3 pt-2 pb-2 btn btn-flat btn-danger float-left " title="محصول جدید">+</a>
              ...
              <table id="dataTable-table" class="table table-striped display nowrap dataTable" style="width:100%;text-align: center;" cellspacing="0">
                ...
                {{-- {{ dd($prayermat) }} --}}
	                <tr>
	                	<td>
	                		<label>
		                    	<input type="checkbox" data-value = "{{$prayermat->id}}" class="flat-red checkbox">
		                  	</label>
	                	</td>
						...
						<td>
							...
							<img src="{{ asset('storage/images/thumbnails/'. $prayermat->images->first()->name)}}" alt="" class="img-circle img-size-50 mr-2">
						...
						<td>
							<div class="br-wrapper br-theme-fontawesome-stars float-left">
								...
								<select class="showGrade{{$key}}" data-value="{{ $prayermat->grades->avg('grade') ?? '0' }}"> 
						...
						<td>
							@if($prayermat->visibility == 0)
								<a class="changeVisibility" href="#" data-id="{{$prayermat->id}}"><i class="fas fa-close danger-color"></i></a> 
							@else
						...
						<td>
							<form class="del-form" action="{{route('prayermat.destroy', [$prayermat])}}" method="post">
								@csrf
								@method('delete')
								<a href="#" data-id="{{$prayermat->id}}" data-model="Prayermat" class="btn btn-outline-danger btn-flat btn-sm delete"><i class="fas fa-trash-alt"></i> حذف </a>
							</form>
						</td>
	                </tr>
	            @endforeach
