...
@section('title', $title ." طرح  ". $prayermat->color_design->design->title ." رنگ  ". $prayermat->color_design->color->color)
      ...
      <div class="col-md-12" style="margin: 0 auto;">
        <div class="card">
          ...
          <div class="card-header">
            <h3 class="card-title">
            	<span id="title"><a href="{{ route('prayermat.storeIndex') }}" style="color: #18d26e;">محصولات سجاده و جانماز</a> / {{ $prayermat->category->title }} طرح {{ $prayermat->color_design->design->title }} رنگ {{ $prayermat->color_design->color->color }}</span>
				...
				@php
          		...
          		<div class="mt-3" style="width: 320px; margin: auto auto;">
          			<div class="card card-sale card-danger card-outline ">
		              	...
		              	<div class="card-body">
		              		<p class="quantity" data-id="{{$prayermat->quantity}}" style="color: #ef3a4e;">
...
{{-- {{ dd($prayermat->favorites->where('user_id',Auth::id())) }} --}}
                ...
                <div class="tab-content">
                  	<div class="tab-pane active" id="tab_1">
                  		<table class="table table-striped table-data">
	                  		<tr>
	                    		<th colspan="2" style="color: #ef3a4e;">{{ $prayermat->category->title }} طرح {{ $prayermat->color_design->design->title }} رنگ {{ $prayermat->color_design->color->color }}</th>
	                  		</tr>
	                  		<tr>
	                  			<td>کد محصول</td>
	                  			<td>{{$prayermat->code}}</td>
	                  		</tr>
	                  		<tr>
	                    		<td>دسته بندی</td>
	                    		<td>{{ $prayermat->category->title }}</td>
	                  		</tr>
	                  		<tr>
	                    		<td>عنوان طرح</td>
	                    		<td>{{ $prayermat->color_design->design->title }}</td>
	                  		</tr>
	                  		<tr>
	                    		<td>تعداد رنگ بافت ترمه</td>
	                    		<td>{{ $prayermat->color_design->design->countOfColor }} رنگ</td>
	                  		</tr>

	                  		<tr>
	                    		<td>رنگ محصول</td>
	                    		<td>{{ $prayermat->color_design->color->color }}</td>
	                  		</tr>

	                  		<tr>
	                    		<td>مشتمل بر</td>
	                    		<td class="multi-line">{{$prayermat->contains}}</td>
	                  		</tr>
	                  		<tr>
	                    		<td>ابعاد محصول</td>
	                    		<td class="multi-line">{{$prayermat->dimensions}}</td>
	                  		</tr>
	                  		<tr>
	                    		<td>وزن تقریبی</td>
	                    		<td class="multi-line">{{$prayermat->weight}}</td>
	                  		</tr>
	                  		<tr>
	                    		<td>جنس محصول</td>
	                    		<td>{{$prayermat->kind}}</td>
	                  		</tr>
	                  		<tr>
	                    		<td>نوع دوخت</td>
	                    		<td>{{$prayermat->sewingType}}</td>
	                  		</tr>
	                  		<tr>
	                    		<td>آستر</td>
	                    		<td>{{$prayermat->haveEster}}</td>
	                  		</tr>
	                  		<tr>
	                    		<td>جنس آستر</td>
	                    		<td>{{$prayermat->kindOfEster}}</td>
	                  		</tr>
	                  		<tr>
	                    		<td>قابلیت شستشو</td>
	                    		<td>{{$prayermat->washable}}</td>
	                  		</tr>
	                  		<tr>
	                    		<td>قیمت محصول</td>
	                  		...
	                  		@can('create', App\Prayermat::class)
		                  		<tr>
		                    		<td>موجودی در انبار</td>
		                    		<td>{{$prayermat->quantity}}</td>
		                  		</tr>
	                  		@endcan
	                  		<tr>
	                    		<td>توضیحات بیشتر</td>
								...
								@if(Auth::check())
									<form action="" method="">
									    ...
									    <div class="img-push">
									    	...
									    	<br>
									  		<input type="submit" id="saveComment" data-model="Prayermat" data-id="{{$prayermat->id}}" value="ثبت" class="btn btn-danger">
									  	</div>
									</form>
								@else
				          	...
				          	@foreach($likePrayermats as $key=>$likePrayermat)
				          		...
				          		<div class="box-product-outer">
						            <div class="box-product">
						                ...
						                <h6>
						                  <a href="{{ route('prayermat.show',[$likePrayermat->id]) }}" class="title">{{ $likePrayermat->category->title }} طرح {{ $likePrayermat->color_design->design->title}} رنگ {{ $likePrayermat->color_design->color->color }}</a>
						                </h6>
						                <div class="price">
						                    @php
						                    	$prices = $likePrayermat->prices->where("local","تومان")->first();
						                    @endphp
						                    <div>
						                      @if($prices->offPrice > 0)
						                ...
						                <div style="clear: both;">
						                  @if($likePrayermat->quantity > 0 and $likePrayermat->quantity <= 5)
	...
	$(function(){
		...
		@foreach($likePrayermats as $key=>$likePrayermat)
	        $('.showGradeLikePrayermat{{$key}}').barrating({
	          ...
	          initialRating: "{{$likePrayermat->grades->avg('grade') ?? '0'}}",
	          readonly:true,
	        });
      	@endforeach
