...
@section("card-title","جزئیات پیام ها")
...
@section("user-content")
	<div class="card">
		<div class="card-body">
			...
			<a href="{{ route('user.messages') }}" class="float-left" >بازگشت <i class="fa fa-chevron-left"></i></a>
		</div>
	</div>
	...
	@if($messageDetails->count() > 0)
		<table class="table border-less text-center">
			...
			@foreach($messageDetails as $key=>$message)	
				<tr class="@if($message->isRead  == 0 and $message->user_id != Auth::id()) text-bold @endif messageDetail  @if($message->user_id != Auth::id()) text-success @endif"  data-id="{{ $message->id }}" style="cursor: pointer;" >
					...
					<td>
						<a href="#" title="حذف پیام" style="font-size: 1.2rem" class="del-message" data-id="{{ $message->id }}"><i class="far fa-trash-alt"></i></a>
					</td>
				</tr>
