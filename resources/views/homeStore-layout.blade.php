        ...
        <ul class="nav-menu">
          ...
          <li class="nav-item dropdown">
  	        <a class="nav-link cart" data-toggle="dropdown" href="{{ route('cart.index') }}" >
  	          ...
  	          @isset($cart)
  	            <span class="badge badge-danger navbar-badge shopping-cart-badge">{{ $sum }}</span>
  	          @endisset
  	        </a>
          ...
          @if(!(Auth::check()))
            <li class="nav-item dropdown">
              <a href="{{ route('login') }}" class="btn btn-flat btn-danger login-icon" style="padding: 5px !important;"> <i class="fa fa-user ml-2"></i> ورود به حساب کاربری / ثبت نام</a>
            </li>
            {{-- <li class="nav-item dropdown">
