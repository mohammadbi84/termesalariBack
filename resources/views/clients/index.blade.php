...
@section('main-content')
    ...
    <div class="card">
        <div class="card-header">
            <a href="{{ route('client.create') }}" class="pr-3 pl-3 pt-2 pb-2 btn btn-flat btn-danger float-left "
                title="محصول جدید">+</a>
            <div class="card-title float-right"><span>مشتریان</span></div>
        </div>
        <div class="card-body">
                ...
                <tbody>
                    @foreach ($clients as $client)
                        <tr>
                            <td>
                                <label>
                                    <input type="checkbox" data-value = "{{ $client->id }}" class="flat-red checkbox">
                                </label>
                            </td>
                            ...
                            <td>
                                <img src="{{asset('storage/'.$client->image)}}" alt="" width="150px">
                            </td>
                            <td>
                                {{ Verta($client->created_at)->format('%d %B %Y') }}
                            </td>
                            <td>
                            ...
                            <td>
                                <form class="del-form" action="{{ route('client.destroy', [$client]) }}"
                                    method="post">
                                    @csrf
                                    @method('delete')
