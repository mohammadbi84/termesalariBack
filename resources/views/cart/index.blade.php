		...
		<div class="card-body product-header-info cart-container row">
				...
				<div class="cart-products col-md-7">
						...
						@foreach($list['products'] as $key=>$product)
							...
							<div class="cart-product row">
								<div class="image-box col-md-2">
									<img class="image-product w-100" src="{{asset('storage/images/thumbnails/'.$product->images->first()->name)}}" style="" alt="{{$product->title}}" >
								</div>
									...
									<div class="row">
										<div>
											<div class="quantity-product">
												<a href="#" class="increase" data-model="{{ $list['models'][$key] }}" data-id = "{{$product->id}}">+</a>
												<span class="count">{{ $list['quantities'][$key] }}</span>
												<a href="#" class="decrease" data-model="{{ $list['models'][$key] }}" data-id = "{{$product->id}}">-</a>
												{{-- <form class="del-form" action="{{route('cart.destroy', [$product->id."-". $cart[$product->id]['moddel'] ])}}" method="post">
													@csrf
													@method('delete') --}}
													<a href="#" data-model="{{ $list['models'][$key]}}" data-id = "{{$product->id}}" class="delete"><i class="far fa-trash-alt"></i></a>
												{{-- </form> --}}
											</div>
										</div>
						...
						<div class="row mt-4 mb-4" id="discountCardRow">
							<div class="col-sm-12 col-md-12 col-lg-12">
								<div class="input-group input-group-md">
				                	<input type="text" name="code" id="code" placeholder="لطفا در صورت داشتن کد تخفیف، آن را وارد کنید." class="form-control" style="font-size: 0.95rem;">
				                  	<span class="input-group-append">
				                    	<button type="button" id="saveDiscountCard" class="btn btn-info btn-flat btn-danger" style="font-size: 0.95rem;">اعمال کد تخفیف</button>
				                  	</span>
				                </div>
							</div>
						</div>
						...
						<div class="row total-row">
							<div class="col-md-6">
								...
								<a href="{{ route('cart.cartlevel2') }}" class="btn btn-sm btn-flat btn-primary">ادامه فرایند خرید</a>
							</div>
							<div class="col-md-6" style="text-align: left">
								<small>مبلغ قابل پرداخت </small>
								
								<p style="padding-top: 5px">
									<b id="total">{{ number_format($price - $discountCardPrice) }}</b> <small> تومان</small>
								</p>
							</div>
						</div>
					{{-- @endisset --}}
				...
				<div class="cart-info col-md-4  table-responsive p-0">	
					<table class="table">
						<tr>
							...
							<td><span id="cart-info-price" data-value="{{ $price + $off }}">{{ number_format($price + $off) }}</span> <small>تومان </small></td>
						</tr>
						<tr>
						...
						<tr style="border-top: 1px solid #ef3a4e">
							<td>جمع </td>
							<td><span id="cart-info-sum" data-value="{{ $price }}">{{number_format($price) ?? ''}}</span> <small>تومان </small></td>
						</tr>
						...
						<tr>
							...
							<td>
								...
								@endphp --}}
								<span id="cart-info-total" data-value="{{ $price - $discountCardPrice }}">{{number_format($price - $discountCardPrice) ?? ''}}</span> <small>تومان </small>
							</td>
						</tr>
					</table>
				</div>
			@else
				<div style="margin: 0 auto; text-align: center;">
					...
					<a href="http://termehsalari.com/shop" class="btn btn-success" >نمایش محصولات <img src="{{asset('/storetemplate/dist/img/online-supermarket-cart.png')}}"></a>
				</div>
...
<script>
	$(function(){
		...
		$('.increase,.decrease').click(function(event){
			...
			var cartQuantity =  $("#cartContainer").children("a[data-id='"+ id +"'][data-moddel='"+ model +"']").find(".cartQuantity").text();
			var totalQuantity = $("#totalQuantity").text();
			var url = "{{url('/cart/change/')}}";
			$.post(url  , { '_token' : '<?php echo csrf_token() ?>', 'action' : action ,'product' : id , 'model' : model}, function(data){
				if (data == "error") {
					swal("خطا  در اجرای عملیات", "اتمام موجودی در انبار" ,"error");
				}
				else
				{
					if(data == "finish")
					...
					if(action == 'decrease'){
						$(".header-total-quantity,#cart_info-quantity,.shopping-cart").text(totalQuantity - 1);
						...
						cartQuantity--;
						$("#cartContainer").children("a[data-id='"+ id +"'][data-moddel='"+ model +"']").find(".cartQuantity").text(cartQuantity);

					}
					else if(action == 'increase'){
						totalQuantity++;
						$(".header-total-quantity,#cart_info-quantity,.shopping-cart").text(totalQuantity);
						$("#cart-info-price").text($.number(parseInt(cartInfoPrice) + parseInt(price)));
						$("#cart-info-off").text($.number(parseInt(cartInfoOff) + parseInt(offprice)));
						$("#cart-info-total,#total,#cart-info-sum,#cartTotalPrice").text(
							$.number(
						...
						//Cart icon in top menu
						$(".shopping-cart-badge").text(parseInt($(".shopping-cart-badge").text())+1);
						$("#totalQuantity").text(totalQuantity);
						cartQuantity++;
						$("#cartContainer").children("a[data-id='"+ id +"'][data-moddel='"+ model +"']").find(".cartQuantity").text(cartQuantity);
					}
			  		...
			  		$.ajax({
			            ...
			            success:function(data){
					        ...
					        else if(data.status == "success")
					        {
					        	...
					        	$(".header-total-quantity,#cart_info-quantity,.shopping-cart").text(totalQuantity - quantity);
							
								$("#cart-info-price").text($.number(cartInfoPrice - (price * quantity) ));
								$("#cart-info-off").text($.number(cartInfoOff - (offprice * quantity) ));
								$("#cart-info-total,#total,#cart-info-sum,#cartTotalPrice").text(
									$.number(
										$("#cart-info-price").text().replace(/,/gi,"") 
										- $("#cart-info-off").text().replace(/,/gi,"")
								...
								//Cart icon in top menu
								$(".shopping-cart-badge").text($(".shopping-cart-badge").text() - quantity);
								totalQuantity = totalQuantity - quantity;
								$("#totalQuantity").text(totalQuantity);
								cartQuantity = cartQuantity - quantity;
								$("#cartContainer").children("a[data-id='"+ id +"'][data-moddel='"+ model +"']").fadeOut('slow');
...
//-----------------------------------------------------------
		$("#saveDiscountCard").click(function(){
			...
			var code = $("#code").val();
			if(code == ""){
				swal("خطا", "لطفا ابتدا کد تخفیف را وارد کنید.","error");
	            ...
	            success:function(data){
	                if(data.res == "success")
	                {
	                	// if (data.type == "price") {
	                		...
	                		$('.cart-info table').find("#cart-info-sum").parents("tr").after("<tr id='discountCardInfo'><td>تخفیف ویژه</td><td>"+ $.number(data.message) +" تومان</td></tr>");
	                	...
	                	// 	var off = (sum * data.amount) / 100 ;
	                	// 	$('.cart-info table').find("#cart-info-sum").parents("tr").after("<tr id='discountCardInfo'><td>تخفیف ویژه</td><td>"+ $.number(off) +" تومان</td></tr>")
	                	// 	$("#cart-info-total").text($.number(total - off));
	                	// 	$("#total").text($.number(total - off));
	                	// 	$("#cart-info-total").attr("data-value", total - off);
	                	// }
	                	swal("عملیات با موفقیت انجام شد.", "کد تخفیف اعمال شد .","success");
	                	$("#code").val("");
	                }
	                else if(data.res == "error"){
	                	swal("خطا در اجرای عملیات", data.message,"error");
	                }
