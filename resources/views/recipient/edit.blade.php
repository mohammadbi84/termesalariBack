...
@section('title','ویرایش اطلاعات گیرنده')

@section('main-content')
<div class="card col-md-6 m-auto">
	<div class="card-header">
    	<h3 class="card-title"><span id="title">ویرایش اطلاعات گیرنده</span></h3>
  	</div>
	<div class="card-body">
		<form class="" id="formAddRecipient" role="form" action="{{route('recipient.update',[$recipient])}}" method="POST">
			@csrf
			@method('put')
			<div class="row">
				<div class="col-6">
						...
						<label for="city_id">استان* </label>
			          	<select tabindex="1" name="city_id" id="city_id" class="form-control select2 select2-hidden-accessible " style="width: 100%;" >
			                ...
			                @foreach($cities as $city)
			                	<option  @if ($city->id == old('city_id') or $recipient->city->id == $city->id) selected @endif value="{{$city->id}}" >{{$city->name}}</option>
			                @endforeach
						...
						<label for="subcity_id">شهر*</label>
			          	<select tabindex="2" name="subcity_id" id="subcity_id" class="form-control select2 select2-hidden-accessible " style="width: 100%;" >
			                ...
			                @foreach($subcities as $subcity)
			                	<option  @if ($subcity->id == old('subcity_id') or $recipient->subcity->id == $subcity->id) selected @endif value="{{$subcity->id}}" >{{$subcity->name}}</option>
			                @endforeach
			          	</select>
			...
			<div class="form-group">
				...
				<input type="text" name="address" id="address" class="form-control @error('address') is-invalid @enderror" placeholder="لطفاً نشانی پستی را وارد کنید." value="{{old('address',$recipient->address)}}">
				@error('address')
				    <div class="invalid-feedback">{{$message}}</div>
				@enderror
			</div>
			...
			<div class="row">
				...
				<div class="col-5">
					<div class="form-group">
						...
						<input type="text" name="houseId" id="houseId" class="form-control @error('houseId') is-invalid @enderror" placeholder="لطفاً پلاک را وارد کنید ." value="{{old('houseId',$recipient->houseId)}}" >
						@error('houseId')
						    <div class="invalid-feedback">{{$message}}</div>
						@enderror
					</div>
				...
				<div class="col-7">
					<div class="form-group">
						...
						<input type="text" name="zipcode" id="zipcode" class="form-control @error('zipcode') is-invalid @enderror" placeholder="لطفاً  کد پستی را وارد کنید ." value="{{old('zipcode',$recipient->zipcode)}}" maxlength="10">
						@error('zipcode')
						    <div class="invalid-feedback">{{$message}}</div>
						@enderror
					</div>
				...
				{{-- <div class="form-group">
					<input type="checkbox" id="recipientIsSelf" name="recipientIsSelf" class=" checkbox" @if($recipient->) @endif data-value = ""> 
			...
			<div class="row">
				...
				<div class="col-6">
					<div class="form-group">
						...
						<input type="text" name="name" id="name" class="form-control @error('name') is-invalid @enderror" placeholder="لطفاً نام گیرنده را وارد کنید ." value="{{old('name',$recipient->name)}}">
						@error('name')
						    <div class="invalid-feedback">{{$message}}</div>
						@enderror
					</div>
				...
				<div class="col-6">
					<div class="form-group">
						...
						<input type="text" name="family" id="recipientfamily" class="form-control @error('family') is-invalid @enderror" placeholder="لطفاً نام خانوادگی گیرنده را وارد کنید ." value="{{old('family',$recipient->family)}}">
						@error('family')
						    <div class="invalid-feedback">{{$message}}</div>
						@enderror
					</div>
			...
			<div class="row">
				...
				<div class="col-6">
					<div class="form-group">
						...
						<input type="text" name="nationalCode" id="nationalCode" class="form-control @error('nationalCode') is-invalid @enderror" placeholder="لطفاً کدملی را وارد کنید ." value="{{old('nationalCode',$recipient->nationalCode)}}" maxlength="10">
						@error('nationalCode')
						    <div class="invalid-feedback">{{$message}}</div>
						@enderror
					</div>
				...
				<div class="col-6">
					<div class="form-group">
						...
						<input type="text" name="mobile" id="mobile" class="form-control @error('mobile') is-invalid @enderror" placeholder="لطفاً شماره مویابل را وارد کنید ." value="{{old('mobile',$recipient->mobile)}}" maxlength="11">
						@error('mobile')
						    <div class="invalid-feedback">{{$message}}</div>
						@enderror
					</div>
			...
			<div>
			    <input type="submit" id="saveRecipient" style="" class="btn btn-primary" value="ویرایش  اطلاعات">		    
			</div>
		</form>
	</div>
</div>
