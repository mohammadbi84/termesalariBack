				...
				<div class="card-body">
					<form class="form" role="form" action="{{ route('design.store') }}" method="post" enctype="multipart/form-data">
		                ...
		                <div class="form-group">
							...
							<input type="text" name="e_title" id="e_title" class="form-control @error('e_title') is-invalid @enderror" placeholder="لطفا عنوان انگلیسی طرح را وارد کنید." value="{{old('e_title')}}">
							@error('e_title')
							    <div class="invalid-feedback">{{$message}}</div>
							@enderror
						</div>
