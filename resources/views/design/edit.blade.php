				...
				<div class="card-body">
					<form class="form" role="form" action="{{ route('design.update',[$design]) }}" method="post" enctype="multipart/form-data">
		                ...
		                <div class="form-group">
							...
							<input type="text" name="ar_title" id="ar_title" class="form-control @error('ar_title') is-invalid @enderror" placeholder="لطفا عنوان انگلیسی طرح را وارد کنید." value="{{ old('ar_title',$design->ar_title) }}">
							@error('ar_title')
							    <div class="invalid-feedback">{{$message}}</div>
							@enderror
						</div>
