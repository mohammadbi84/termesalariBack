    ...
    <section class="content">
        ...
        <div class="card col-md-12 col-sm-12">
            ...
            <div class="card-body">
                ...
                <form class="form" role="form" action="{{ route('category.update', [$category]) }}" method="post"
                        ...
                        <select name="parent_id" id="parent_id" class="form-control" style="width: 100%;">
                            @foreach ($productCategories as $productCategory)
                                <option value="{{ $productCategory->id }}"
                                    @if ($productCategory->id == old('parent_id', $category->parent_id)) selected @endif>{{ $productCategory->title }}
                                </option>
                            @endforeach
                        </select>
                        @error('parent_id')
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" name="e_title" id="e_title"
                            ...
                            value="{{ old('e_title', $category->e_title) }}">
                        @error('e_title')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="form-group @error('image') is-invalid @enderror">
                        ...
                        <div class="file-loading">
                            <input id="image" name="image" type="file" data-browse-on-zone-click="true"
                                data-show-caption="true" value="{{ $category->image }}">
                        </div>
                        @error('image')
