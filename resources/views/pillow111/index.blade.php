...
@section('title','پنل مدیریت | محصولات پشتی و کوسن')
      ...
      <div class="row">
        <div class="col-12">
            ...
            <div class="card-header">
              	<a href="{{ route('pillow.create') }}" class="pr-3 pl-3 pt-2 pb-2 btn btn-flat btn-danger float-left " title="محصول جدید">+</a>
              	<h3 class="card-title float-right">
              		<span>
              			لیست محصولات پشتی و کوسن
              		</span>
              	</h3>
            </div>
            <!-- /.card-header -->
                ...
                @if(count($pillows)>0)
					...
					{{-- {{ dd($pillow) }} --}}
						<tr>
							<td>
								<label>
									<input type="checkbox" data-value = "{{$pillow->id}}" class="flat-red checkbox">
								</label>
							</td>
							...
							<td>
								...
								<img src="{{ asset('storage/images/thumbnails/'. $pillow->images->first()->name)}}" alt="" class="img-circle img-size-50 mr-2">
							...
							<td>
								<div class="br-wrapper br-theme-fontawesome-stars float-left">
									...
									<select class="showGrade{{$key}}" data-value="{{ $pillow->grades->avg('grade') ?? '0' }}"> 
							...
							<td>
								@if($pillow->visibility == 0)
									<a class="changeVisibility" href="#" data-id="{{$pillow->id}}"><i class="fas fa-close danger-color"></i></a> 
								@else
									<a class="changeVisibility" href="#" data-id="{{$pillow->id}}"><i class="fas fa-check success-color"></i> </a>
								@endif
							</td>
							<td>
								{{-- <a href="{{ route('pillow.duplicate',[$pillow]) }}"><i class="fas fa-copy text-primary"></i></a> --}}
								<a href="{{ route('pillow.duplicate',[$pillow]) }}" class="btn btn-outline-secondary btn-flat btn-sm"><i class="fas fa-copy"></i> کپی  </a>
							</td>
							<td><a href="{{route('pillow.show',[$pillow])}}" target="blank" class="btn btn-outline-info btn-flat btn-sm"><i class="fas fa-info-circle"></i> جزئیات </a></td>
							<td><a href="{{route('pillow.edit',[$pillow])}}" class="btn btn-outline-primary btn-flat btn-sm"><i class="fas fa-edit"></i> ویرایش </a></td>
							<td><a href="{{route('comment.product',['Pillow',$pillow->id])}}" class="btn btn-outline-primary btn-flat btn-sm"><i class="fas fa-comment"></i> نظرات </a></td>

							<td>
								<form class="del-form" action="{{route('pillow.destroy', [$pillow])}}" method="post">
									@csrf
									@method('delete')
									<a href="#" data-id="{{$pillow->id}}" data-model="Pillow" class="btn btn-outline-danger btn-flat btn-sm delete"><i class="fas fa-trash-alt"></i> حذف </a>
								</form>
							</td>
						</tr>
					@endforeach
...
//-------------------Show Grade-----------------------------
	{{-- @foreach($pillows as $key=>$pillow)
		$('.showGrade{{$key}}').barrating({
		 	theme:'fontawesome-stars-o',
			initialRating: "{{$pillow->grades->avg('grade') ?? '0'}}",
		 	readonly:true,
		});
	@endforeach --}}
//-------------------Delete Product------------------
	...
	$(document).on('click', '.changeVisibility', function(event){
		event.preventDefault();
		var url = "{{ route('pillow.changeVisibility') }}";
		var id = $(this).data("id");
		var $thiz = $(this);
		$.ajax({
		    type: 'POST',
...
//----------change Statuse group--------------------------------
	$('#changeVisibilityGroup').click(function(event){
		event.preventDefault();
		var items = [];
		var url = "{{ route('pillow.changeVisibilityGroup') }}"
