...
@section('title', $title ." طرح  ". $pillow->color_design->design->title ." رنگ  ". $pillow->color_design->color->color)
      ...
      <div class="col-md-12" style="margin: 0 auto;">
        <div class="card">
          ...
          <div class="card-header">
            <h3 class="card-title">
            	<span id="title"><a href="{{ route('pillow.storeIndex') }}" style="color: #18d26e;">محصولات پشتی و کوسن</a> / {{ $pillow->category->title }} طرح {{ $pillow->color_design->design->title }} رنگ {{ $pillow->color_design->color->color }}</span>
				...
				@php
          		...
          		<div class="mt-3" style="width: 320px; margin: auto auto;">
          			<div class="card card-sale card-danger card-outline ">
		              	...
		              	<div class="card-body">
		              		<p class="quantity" data-id="{{$pillow->quantity}}" style="color: #ef3a4e;">
		              			@if($pillow->quantity > 0 and $pillow->quantity <= 5)
		              				<i class="fas fa-bell"></i>
		              			@endif
		              			@if($pillow->quantity == 0)
		              				<span class="text-bold"> اتمام موجودی در انبار </span>
			              		@elseif($pillow->quantity <= 5)
		                	...
		                	<p>	
		                	<div class="row">
		                		<input @if($pillow->quantity > 0) id="addToCart" @endif type="submit" value="افزودن به سبد خرید" class="btn btn-block btn-danger btn-lg @if($pillow->quantity==0) disabled @endif" data-id = "{{$pillow->id}}" data-moddel = "Pillow" data-design="{{ $pillow->color_design->design->title }}" data-color="{{ $pillow->color_design->color->color }}" data-title="{{ $pillow->title }}" data-price="{{ $prices->price }}" data-pay="{{ $price }}" data-off="{{ $off }}" data-local="{{ $prices->local }}" >
		                	</div>
		                	</p>

							<div class="br-wrapper br-theme-fontawesome-stars">
	                				امتیاز : 
							  <select id="showGrade" data-model="Pillow" data-current-rating="4"> <!-- now hidden -->
...
{{-- {{ dd($pillow->favorites->where('user_id',Auth::id())) }} --}}

							<a href="#" id="addToFavorite" data-id="{{ $pillow->id }}" data-model="Pillow" class="float-left mt-1" title="افزودن به لیست علاقه مندی ها" style="font-size: 1.5rem"><i class="@if($pillow->favorites->where('user_id',Auth::id())->count() > 0 ) fa fa-heart @else far fa-heart @endif"></i></a>

							<a href="" id="compare" title="افزودن به لیست مقایسه" class="float-left ml-2 mt-2" data-id="{{ $pillow->id }}" data-model="Pillow" style="vertical-align: middle;"><img src="{{ asset('/storetemplate/dist/img/compare.png') }}"></a>
                ...
                <div class="tab-content">
                  	<div class="tab-pane active" id="tab_1">
                  		<table class="table table-striped table-data">
	                  		<tr>
	                    		<th colspan="2" style="color: #ef3a4e;">{{ $pillow->category->title }} طرح {{ $pillow->color_design->design->title }} رنگ {{ $pillow->color_design->color->color }}</th>
	                  		</tr>
	                  		<tr>
	                  			<td>کد محصول</td>
	                  			<td>{{$pillow->code}}</td>
	                  		...
	                  		<tr>
	                    		...
	                    		<td>{{ $pillow->color_design->design->countOfColor }} رنگ</td>
	                  		</tr>

	                  		<tr>
	                    		<td>رنگ محصول</td>
	                    		<td>{{ $pillow->color_design->color->color }}</td>
	                  		</tr>

	                  		<tr>
	                    		<td>ابعاد محصول</td>
	                    		<td class="multi-line">{{$pillow->dimensions}}</td>
	                  		</tr>
	                  		<tr>
	                    		<td>وزن تقریبی</td>
	                    		<td class="multi-line">{{$pillow->weight}}</td>
	                  		</tr>
	                  		<tr>
	                    		<td>جنس محصول</td>
	                    		<td>{{$pillow->kind}}</td>
	                  		...
	                  		<tr>
	                    		<td>توضیحات بیشتر</td>
	                    		<td class="multi-line">{{$pillow->description}}</td>
	                  		</tr>
			            ...
			            {{-- <div class="card card-widget"> --}}
			              ...
			              {{-- <div class="card-body"> --}}
			                ...
			                <div class="br-wrapper br-theme-fontawesome-stars" style="margin-bottom: 20px;">
							  ...
							  <select id="setGrade" data-model="Pillow" data-id="{{ $pillow->id }}"> <!-- now hidden -->
							    <option value="1">1</option>
							    <option value="2">2</option>
							    <option value="3">3</option>
							    <option value="4">4</option>
								...
								@if(Auth::check())
									<form action="" method="">
									    ...
									    <div class="img-push">
									    	...
									    	<br>
									  		<input type="submit" id="saveComment" data-model="Pillow" data-id="{{$pillow->id}}" value="ثبت" class="btn btn-danger">
									  	</div>
									</form>
								@else
	          	...
	          	<div class="card">
	              	...
	              	<div class="">
	              		<div class="owl-carousel owl-theme listCarousel carousel-fade card-body" id="carouselItems">
				          	@foreach($likePillows as $key=>$likePillow)
				          		
				          		<div class="box-product-outer">
						            <div class="box-product">
						                <a href="{{ route('pillow.show',[$likePillow->id]) }}">
						                	<div class="img-wrapper mb-2">
						                      	<img alt="Product" src="{{asset('/storage/images/thumbnails/'.$likePillow->images->sortby('ordering')->first()->name)}}" class="w-100">
						                	</div>
						                </a>
						                <h6>
						                  <a href="{{ route('pillow.show',[$likePillow->id]) }}" class="title">{{ $likePillow->category->title }} طرح {{ $likePillow->color_design->design->title}} رنگ {{ $likePillow->color_design->color->color }}</a>
						                </h6>
						                <div class="price">
						                    @php
						                    	$prices = $likePillow->prices->where("local","تومان")->first();
						                    @endphp
						                    <div>
						                      @if($prices->offPrice > 0)
						                ...
						                <small class="text-muted">{{ count($likePillow->grades) }} نفر</small>
						                <div class="br-wrapper br-theme-fontawesome-stars float-left">
						                  <select class="showGradeLikePillow{{$key}}"> 
						                      <option value="1">1</option>
						                      <option value="2">2</option>
						                      <option value="3">3</option>
						                      <option value="4">4</option>
						                ...
						                <div style="clear: both;">
						                  @if($likePillow->quantity > 0 and $likePillow->quantity <= 5)
						                  <i class="fas fa-bell" style="color: #ef3a4e"></i>
						                  @endif
						                  <small @if($likePillow->quantity > 5) style="color: #000" @else style="color: #ef3a4e" @endif>
						                  @if($likePillow->quantity == 0) اتمام موجودی در انبار 
						                    @elseif($likePillow->quantity <= 5)کمتر از 5 عدد موجود می باشد  .
						                  @endif
						                  </small>
						                </div>
...
@push('js')
	...
	$(function(){
		...
		@foreach($likePillows as $key=>$likePillow)
	        $('.showGradeLikePillow{{$key}}').barrating({
	          theme:'fontawesome-stars-o',
	          initialRating: "{{$likePillow->grades->avg('grade') ?? '0'}}",
	          readonly:true,
	        });
      	@endforeach
