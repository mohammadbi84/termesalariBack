...
<div class="row">
	<div class="col">
	    ...
	    <div class="card">
	      ...
	      <!-- form start -->
	      {{-- @dump($errors) --}}
	      <form role="form" id="insertForm" action="{{ route('pillow.store') }}" method="POST" enctype="multipart/form-data">
	      	@csrf
	        <div class="card-body">

	        	<div class="form-group">
                	<label for="code">کد محصول</label>
					<input type="text" name="code" class="form-control @error('code') is-invalid @enderror" placeholder="کد محصول را وارد کنید ." value="{{old('code',$pillow->code)}}" autofocus="autofocus">
					@error('code')
					...
					<label for="category_id">دسته بندی محصول</label>
                  	<select name="category_id" id="category_id" class="form-control select2 select2-hidden-accessible " style="width: 100%;">
                  		...
                  		@foreach($categories as $category)
                  			<option value="{{ $category->id }}" @if($category->id == old('category_id',$pillow->category->id)) selected @endif >{{ $category->title }}</option>
                  		@endforeach
                  	</select>
					...
					<label for="design_id">عنوان  طرح</label>
                  	<select  name="design_id" id="design_id" class="form-control select2 select2-hidden-accessible " style="width: 100%;" >
	                    ...
	                    @foreach($designs as $design)
	                    	<option @if (old('design_id', $pillow->color_design->design->id) == $design->id ) selected @endif value="{{$design->id}}">{{$design->title}}</option>
	                    @endforeach
				...
				<div class="form-group">
                    ...
                    <select name="color_id" id="color_id" class="form-control @error('color_id') is-invalid @enderror">
                    	@foreach($colors as $color)
                    		<option @if (old('color_id', $pillow->color_design->color->id) == $color->id ) selected @endif value="{{ $color->id }}">{{ $color->color }}</option>
                    	@endforeach
                    </select>
                    @error('color_id')
					    <div class="invalid-feedback">{{$message}}</div>
				...
				<div class="form-group">
                    ...
                    <select name="kind" class="form-control @error('kind') is-invalid @enderror">
                    	{{-- <option value="">جنس محصول را انتخاب کنید .</option> --}}
                      	<option  @if (old('kind',$pillow->kind) == 'ابریشم مصنوعی (ویسکوز ریون)') selected @endif value="ابریشم مصنوعی (ویسکوز ریون)">ابریشم مصنوعی (ویسکوز ریون)</option>
						<option  @if (old('kind',$pillow->kind) == 'ابریشم مصنوعی') selected @endif value="ابریشم مصنوعی">ابریشم مصنوعی</option>
                    </select>
                    @error('kind')
					    <div class="invalid-feedback">{{$message}}</div>
					@enderror
				...
				{{-- <div class="form-group">
                    ...
                    <select name="washable" class="form-control @error('washable') is-invalid @enderror">
                    	<option value="">قابلیت شستشو</option>
                      	<option @if (old('washable',$pillow->washable) == 'توسط دست') selected @endif value="توسط دست">توسط دست</option>
						<option @if (old('washable',$pillow->washable) == 'توسط ماشین لباسشویی') selected @endif value="توسط ماشین لباسشویی">توسط ماشین لباسشویی</option>
						<option @if (old('washable',$pillow->washable) == 'ندارد') selected @endif value="ندارد">ندارد</option>
                    </select>
                    @error('washable')
					    <div class="invalid-feedback">{{$message}}</div>
					@enderror
...
{{-- @dump(session()->all()) --}}
                ...
                <div class="form-group">
					...
					<input type="text" name="quantity" class="form-control @error('quantity') is-invalid @enderror" placeholder="مثلاً 200" value="{{old('quantity',$pillow->quantity)}}">
					@error('quantity')
					    <div class="invalid-feedback">{{$message}}</div>
					@enderror
				</div>
				...
				success: function(data)
				{
					if(data.length > 0){
						...
						data.forEach((element) => {
							if("{{ old('color_id', $pillow->color_design->color->id) }}" == element.id)
